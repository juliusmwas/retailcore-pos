...
function Login() {
  ...
  useEffect(() => {
    ...
    if (storedAuth) {
      ...
      const role =
        parsed.user?.role || parsed.user?.branches?.[0]?.role || "OWNER";

      // --- REPLACE THIS BLOCK INSIDE YOUR useEffect ---
      if (role === "OWNER" || role === "ADMIN") {
        const hasBranches =
          parsed.user?.branches?.length > 0 || parsed.branches?.length > 0;
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-blue-100 p-4">
      <div className="w-full max-w-lg bg-white p-8 rounded-xl shadow-lg">
        ...
        {error && (
          <p className="bg-red-50 text-red-500 p-3 rounded-lg text-sm mb-4 text-center border border-red-100">
            {error}
          </p>
        )}
        ...
        <a
          href="/"
          className="flex items-center gap-2 group text-slate-500 text-center  mt-5 hover:text-blue-600 transition-colors"
        >
          <ArrowLeft
            size={18}
