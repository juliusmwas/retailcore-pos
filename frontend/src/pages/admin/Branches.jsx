...
export default function Branches() {
    ...
    try {
      ...
      const payload = {
        ...
        openingDate: branchForm.openingDate
          ? new Date(branchForm.openingDate).toISOString()
  ...
  const baseInputClass =
    "w-full px-4 py-2.5 bg-gray-50 border rounded-xl transition-all outline-none text-gray-700 focus:ring-2";
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
            ...
            <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
              ...
              <p className="text-3xl font-bold text-blue-600 mt-1">
                {branches.reduce((acc, b) => acc + (b._count?.users || 0), 0)}
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {branches.map((branch) => (
              <div
                key={branch.id}
                className="group bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-xl hover:border-blue-200 transition-all p-6"
              >
                  ...
                  <div className="flex gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500 flex items-center gap-1">
                        <MapPin size={14} /> {branch.city}, {branch.country}
                      </p>
                    </div>
                  </div>
                  <span
                    className={`px-3 py-1 rounded-full text-[10px] font-bold ${branch.status === "ACTIVE" ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}
                  >
                    {branch.status}
                  </span>
                </div>
                ...
                <div className="flex gap-2 mt-4">
                  ...
                  <button
                    ...
                    className="p-2.5 rounded-xl border border-gray-100 text-gray-400 hover:text-red-600 hover:bg-red-50 transition-all"
                  >
                    <Power size={18} />
                  </button>
                </div>
        ...
        <div className="flex flex-col items-center justify-center py-24 bg-white rounded-3xl border-2 border-dashed border-gray-100 text-center animate-in fade-in zoom-in duration-500">
          ...
          <p className="text-gray-500 max-w-sm mt-2 mb-10">
            Every great business starts with its first location. Click below to
            register your headquarters or first branch.
          </p>
          <button
            onClick={() => setShowAddModal(true)}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
          <div className="bg-white w-full max-w-4xl rounded-2xl shadow-2xl overflow-hidden flex flex-col animate-in zoom-in duration-200">
            <div className="px-8 py-6 border-b border-gray-100">
              ...
              <div className="flex gap-2">
                {[1, 2, 3, 4, 5, 6].map((s) => (
                  <div
                    key={s}
                    className={`h-1.5 flex-1 rounded-full ${step >= s ? "bg-blue-600" : "bg-gray-100"}`}
                  />
                ))}
              </div>
            </div>
            ...
            <div className="px-8 py-8 max-h-[60vh] overflow-y-auto">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setBranchForm((p) => ({
                        ...p,
                        name: e.target.value,
                        code: p.code || generateCode(e.target.value),
                      }))
                    }
                  />
                  <input
                    className={`${baseInputClass} font-mono bg-blue-50/30`}
                    name="code"
              ...
              {step === 2 && (
                <div className="space-y-4 animate-in slide-in-from-right-4">
                  ...
                  <input
                    className={getInputClass(branchForm.country, true)}
                    name="country"
                    placeholder="Country *"
                    onChange={handleChange}
                  ...
                  />
                  <input
                    className={getInputClass(branchForm.city, true)}
                    name="city"
                    placeholder="City *"
                    onChange={handleChange}
                  ...
                  />
                  <input
                    className={getInputClass(branchForm.address, true)}
                    name="address"
              ...
              {step === 3 && (
                <div className="space-y-4 animate-in slide-in-from-right-4">
                  ...
                  <input
                    className={getInputClass(branchForm.managerName, true)}
                    name="managerName"
                  ...
                  />
                  <input
                    className={getInputClass(branchForm.managerEmail, true)}
                    name="managerEmail"
                  ...
                  />
                  <input
                    className={getInputClass(branchForm.managerPhone, true)}
                    name="managerPhone"
              ...
              {step === 5 && (
                <div className="space-y-4 animate-in slide-in-from-right-4">
                  ...
                  <input
                    className={getInputClass(branchForm.currency, true)}
                    name="currency"
                ...
                <div className="space-y-4 animate-in zoom-in-95">
                  <div className="bg-blue-50 p-4 rounded-xl border border-blue-100">
                    ...
                    <p className="text-sm text-blue-700">
                      Check all details before creating the branch.
                    </p>
                  </div>
            ...
            <div className="px-8 py-6 border-t border-gray-100 bg-gray-50 flex justify-between">
              <button
                onClick={step === 1 ? handleCloseModal : prevStep}
                className="px-6 py-2 rounded-xl border border-gray-300 font-bold text-gray-600 hover:bg-white transition-all"
              >
                {step === 1 ? "Cancel" : "Back"}
              </button>
              {step < 6 ? (
                <button
                  ...
                  className={`px-8 py-2 rounded-xl font-bold transition-all ${isStepValid() ? "bg-blue-600 text-white" : "bg-gray-200 text-gray-400 cursor-not-allowed"}`}
                >
                  Continue
                </button>
              ) : (
                <button
                  ...
                  className="px-8 py-2 bg-green-600 text-white rounded-xl font-bold hover:bg-green-700 transition-all"
                >
