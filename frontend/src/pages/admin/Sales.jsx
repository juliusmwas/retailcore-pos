  ...
  const fetchStats = async () => {
    try {
      const response = await axios.get(
        `/api/sales/stats?businessId=${businessId}&branchId=${selectedBranch}`,
      );
    ...
    <div className="p-8 bg-[#f8fafc] min-h-screen space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 font-medium">
            Real-time transaction monitoring across all active branches
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-4">
        <div className="relative flex-1 group">
          ...
          {searchQuery && (
            <button
              ...
              className="absolute right-4 top-1/2 -translate-y-1/2 text-[10px] font-bold text-gray-400 hover:text-red-500 uppercase tracking-widest"
            >
              Clear
            </button>
          )}
        ...
        <button
          onClick={() =>
            alert(
              "Filter drawer coming soon! Here you'll filter by Date, Status, or Payment Method.",
            )
          }
      ...
      <div className="space-y-4">
        ...
        <div className="hidden md:grid grid-cols-6 px-8 text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em]">
          <div>Transaction</div>
          <div>Location</div>
          <div>Cashier</div>
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {[1, 2, 3].map((n) => (
              <div
                key={n}
                className="h-24 w-full bg-white animate-pulse rounded-[2rem] border border-gray-50"
              />
            ))}
          </div>
          ...
          filteredSales.map((sale) => (
            ...
            >
              ...
              <div className="flex flex-col">
                <span className="text-xs font-bold text-blue-600 font-mono tracking-tighter uppercase leading-none">
                  #{sale.id}
                </span>
              ...
              <div className="text-center">
                <span className="px-3 py-1 bg-gray-100 text-[9px] font-bold rounded-lg text-gray-500 uppercase tracking-wider">
                  {sale.payment}
                </span>
              </div>
              ...
              <div className="flex items-center justify-end gap-4">
                <span
                  className={`px-4 py-1.5 rounded-xl text-[10px] font-bold border tracking-tighter ${
